...
var escapeModelName = require('./generator-base').escapeModelName;
...
function buildModel(models, schema, jsonModel, modelName, anonymous) {
  modelName = escapeModelName(modelName);
  if (models[modelName]) {
  ...
  if (Array.isArray(jsonModel.allOf)) {
    ...
    for (var i = 0, n = jsonModel.allOf.length; i < n; i++) {
      ...
      if (getRef(item)) {
        ...
        var refModelName = getRef(item).substring('#/definitions/'.length);
        base = models[escapeModelName(refModelName)] || base;
        refs.push(base);
      } else {
    ...
    if (refs.length === 1) {
      ...
      model.base = (base && base.name && escapeModelName(base.name)) || base;
    } else {
