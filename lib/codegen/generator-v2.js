...
var buildModels = require('./json-schema');
...
V2Operation.prototype.parameter = function(p) {
  ...
  if (p.schema && getRef(p.schema)) {
    ...
    type = (this.name + '_' + p.name).replace(/[^\w]+/g, '_');
    var schema = {};
    schema[type] = p.schema;
    var models = buildModels(schema);
    var typeName = base.escapeModelName(type);
    this.models[type] = models[typeName];
    return {
      arg: p.name,
      type: typeName || 'any',
  ...
  for (code in this.responses) {
    ...
    if (code.match(/^2\d\d$/)) {
      if (res.schema && getRef(res.schema)) {
        ...
        let typeName = base.escapeModelName(type);
        this.returnType = typeName || 'any';
        returns.push({
        ...
        res.schema.items) {
        if (res.schema.items.type === 'object') {
          type = (this.name + '_response_' + code).replace(/[^\w]+/g, '_');
          var schema = {};
          schema[type] = res.schema.items;
          var models = buildModels(schema);
          this.models[type] = models[base.escapeModelName(type)];
        } else {
          type = res.schema.items.type;
        }
        let typeName = base.escapeModelName(type);
        this.returnType = [typeName];
        returns.push({
      ...
      } else if (res.schema && res.schema.type === 'object') {
        type = (this.name + '_response_' + code).replace(/[^\w]+/g, '_');
        schema = {};
        schema[type] = res.schema;
        models = buildModels(schema);
        let typeName = base.escapeModelName(type);
        this.models[type] = models[typeName];
        this.returnType = typeName;
        returns.push({
  ...
  for (var path in spec.paths) {
    ...
    for (var verb in ops) {
      ...
      op.parameters.forEach(p => {
        ...
        if (p.in === 'path') {
          p.name = paramName.replace(/[^\w]+/g, '_');
        }
      });
      ...
      const opPath = path.replace(/\{[^\}]+\}/g,
        varName => varName.replace(/[^\w\{\}]+/g, '_'));
      // Replace {id} with :id
      op.path = opPath.replace(/{(([^{}])+)}/g, ':$1');
      ...
      if (index !== -1) {
        ...
        if (op.name.indexOf('prototype.') === 0) {
          op.name = 'prototype.' + base.escapeIdentifier(op.name.substring(10));
        } else {
          op.name = base.escapeIdentifier(op.name);
        }
      } else {
        op.name = base.escapeIdentifier(op.name);
      }
