 ...
 * @returns {{jsonProperty: *, property: {}, type: *}}
 */
...
function buildModel(models, jsonModel, modelName) {
  if (models[modelName]) {
  ...
  if (Array.isArray(jsonModel.allOf)) {
    ...
    for (var i = 0, n = jsonModel.allOf.length; i < n; i++) {
      ...
      if (item.$ref) {
        ...
        itemModel = buildModel(models, item, modelName + '_' + i);
      }
      if (itemModel) {
        // Add more item model properties to the model
        for (var prop in itemModel.properties) {
